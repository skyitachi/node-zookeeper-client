...
function ConnectionManager(connectionString, options, stateListener) {
    ...
    // 连接zookeeper server的重试次数
    // Note: check if isNaN 
    this._connectionRetryRemaining = (1 + options.connectRetry) * this.servers.length;
...
ConnectionManager.prototype.onSocketClosed = function (hasError) {
    ...
    if (this.beforeConnected) {
        if (this._connectionRetryRemaining > 0) {
            this.connect();
        }
        return;
    }
...
ConnectionManager.prototype.onSocketError = function (error) {
    ...
    if (this.beforeConnected && !this._connectionRetryRemaining) {
        // 未能建立连接的时候抛出错误
        // Note: error type
        this.emit("error", new Error(`cannot connect ${this.connectionString}`));
    }
};
